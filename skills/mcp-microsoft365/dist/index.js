...
async function handleTool(name, args) {
    ...
    switch (name) {
        ...
        case "m365_mail_list": {
            ...
            return JSON.stringify(data.value.map((m) => ({
                id: m.id,
                subject: m.subject,
        ...
        case "m365_mail_send": {
            ...
            if (args.cc) {
                message.ccRecipients = args.cc.split(",").map((email) => ({
                    emailAddress: { address: email.trim() },
                }));
            }
        ...
        case "m365_mail_search": {
            const data = await graphRequest(`/users/${user}/messages?$search="${encodeURIComponent(args.query)}"&$top=${args.top || 10}&$select=id,subject,from,receivedDateTime,bodyPreview`);
        ...
        case "m365_calendar_list": {
            ...
            const data = await graphRequest(endpoint);
            return JSON.stringify(data.value.map((e) => ({
                id: e.id,
                subject: e.subject,
                start: e.start,
                end: e.end,
        ...
        case "m365_calendar_create": {
            ...
            if (args.body)
                event.body = { contentType: "HTML", content: args.body };
            if (args.location)
        ...
        case "m365_calendar_availability": {
            const schedules = args.users.split(",").map((u) => u.trim());
        ...
        case "m365_files_list": {
            ...
            const data = await graphRequest(endpoint);
            return JSON.stringify(data.value.map((f) => ({
                id: f.id,
                name: f.name,
                size: f.size,
                isFolder: !!f.folder,
        ...
        case "m365_tasks_lists": {
            ...
            return JSON.stringify(data.value.map((l) => ({
                id: l.id,
        ...
        case "m365_tasks_create": {
            ...
            if (args.dueDateTime)
                task.dueDateTime = { dateTime: args.dueDateTime, timeZone: "UTC" };
        ...
        case "m365_teams_chats": {
            ...
            return JSON.stringify(data.value.map((c) => ({
                id: c.id,
                topic: c.topic,
                chatType: c.chatType,
...
const server = new Server({ name: "mcp-microsoft365", version: "1.0.0" }, { capabilities: { tools: {} } });
